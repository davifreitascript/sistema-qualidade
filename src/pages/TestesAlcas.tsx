...
import { TabelaAlcas } from "../components/TabelaAlcas";
import { ModalExcluirAlcas } from "../components/modalExcluirAlcas";
import { sincronizarTestes, excluirTeste } from "../services/alcas";
...
export default function TestesAlcas() {
  ...
  const [testes, setTestes] = useState<TesteAlcas[]>(() => {
    const dadosSalvos = localStorage.getItem("testes-alca");
    return dadosSalvos ? JSON.parse(dadosSalvos) : [];
  });

  const [statusSync, setStatusSync] = useState<"idle" | "loading" | "success">("idle");
  const [testeSelecionado, setTesteSelecionado] = useState<TesteAlcas | null>(null);
  ...
  function editarTeste(teste: TesteAlcas) {
    console.log("Editar teste:", teste);
  }

  async function solicitarExclusao(teste: TesteAlcas, numero: number) {
