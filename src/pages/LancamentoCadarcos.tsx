...
import { formInicial } from "../types/cadarcos";
import { FormularioCadarcos } from "../components/FormularioCadarcos";
import { obterDataAtual} from "../utils/formatarData";
import { gerarLotePorData } from "../utils/gerarLote";
import { salvarTesteLocal } from "../services/cadarcos";
import { CHAVE_FORMULARIO } from "../config/auth";
import toast from "react-hot-toast";

export default function LancamentoAlcas() {

  const [form, setForm] = useState<FormTesteCadarcos>(() => {
    const hoje = obterDataAtual();

    const salvo = sessionStorage.getItem(CHAVE_FORMULARIO);

    if (salvo) {
      const formulario: FormTesteCadarcos = JSON.parse(salvo);
  ...
  async function salvarTeste(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const invalidos = obrigatorios.filter(
      (campo) => !form[campo as keyof FormTesteCadarcos]
    );
  ...
  return (
    <div className="flex flex-col justify-center items-center pt-6 bg-slate-100">
      ...
      <div className="mx-auto w-full max-w-5xl">
        <h1 className="my-8 text-center font-bold text-slate-800 md:text-5xl text-3xl select-none">Lançamento de cadarços</h1>
